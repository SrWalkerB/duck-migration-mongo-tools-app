...
export function StepSelectSource({ state, setSource }: StepSelectSourceProps) {
  ...
  useEffect(() => {
    ...
    getDatabases(state.sourceConnectionId)
      .then((data) => {
  ...
  const handleConnectionChange = (connectionId: string) => {
    setSource(connectionId, '');
  };

  const handleDatabaseChange = (database: string) => {
    setSource(state.sourceConnectionId, database);
  };

  return (
    <div className="space-y-6">
      <div className="space-y-2">
        <h2 className="text-xl font-semibold text-foreground">Select Source</h2>
        <p className="text-sm text-muted-foreground">
          Choose the source MongoDB connection and database to migrate from.
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Select the MongoDB server to read from</CardDescription>
        </CardHeader>
        <CardContent>
          {loadingConnections ? (
      ...
      {state.sourceConnectionId && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Select the database to migrate</CardDescription>
          </CardHeader>
          <CardContent>
            {loadingDatabases ? (
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
